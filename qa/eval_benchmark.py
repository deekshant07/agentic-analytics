...
def build_100_question_suite(catalog: dict) -> list[EvalCase]:
    # Only include metrics that will be in the runtime metrics list (approved/no-status).
    # Excluded: "rejected" or "suppressed" metrics — the compiler won't find them and
    # compile_query returns "" which is an avoidable false-negative.
    _EXCLUDED_STATUSES = {"rejected", "suppressed"}
    metrics = []
    for tname, tdata in catalog.items():
        ...
        for m in tdata.get("suggested_metrics", []):
            ...
            if mid and m.get("status") not in _EXCLUDED_STATUSES:
                metrics.append(mid)
    metrics = sorted(set(metrics))
    months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun"]
    cases: list[EvalCase] = []

    # Infer the correct expected analysis_type for metrics that route to specialist paths.
    # The orchestrator correctly returns "retention" for retention metrics, "funnel" for
    # funnel metrics, etc. — the eval must reflect this rather than blindly expecting "metric".
    ...
    def _expected_type_for_metric(mid: str) -> str:
        mid_lower = mid.lower()
        for pattern, atype in _SPECIALIST_PATTERNS:
            if pattern in mid_lower:
                return atype
        return "metric"

    # 1) Metric trend/rate questions (40)
    for i, mid in enumerate(metrics[:20]):
        exp_type = _expected_type_for_metric(mid)
        cases.append(EvalCase(
            question=f"show {mid.replace('_', ' ')} for {months[i % len(months)]}",
    ...
    # Raw event breakdowns (transacting users, onboarding completed) route to "custom_segment".
    ...
    # 3) Comparison/custom split questions (20)
    split_qs = [
        "Share Jupiter active users vs Transacting users split for Jan",
        "Compare in app transacting users vs off app transacting users for Jan",
        "Jupiter active user versus transacting user for Feb",
        "difference between jupiter active users and transacting users in Mar",
    ]
    for i in range(20):
        cases.append(EvalCase(
            question=split_qs[i % len(split_qs)],
            expected_route="custom_split",
            expected_columns_any=["cohort", "users"],
            tags=["custom_split", "comparison", "cohort"],
        ))
    ...
    # These catch compiler bugs that produce zero rows without a SQL syntax error.
    # Each assertion names the EXACT structural invariant that was broken in production.
    cases += [
        # Bug fix #1: filters.date list must NOT render as AND date = '[...]'
        EvalCase(
            question="show activation rate for Jan",
            expected_analysis_type="metric",
            tags=["sql_structure", "filter_rendering"],
            gold_sql={
                "forbidden": ["AND date =", "date = '['"],
            },
        ),
        # Bug fix #2: behavioral_cohort event-specific filters must NOT appear on did_a CTE
        EvalCase(
            question="How many users did onboarding and UPI transactions in Feb",
            expected_analysis_type="behavioral_cohort",
            tags=["sql_structure", "behavioral_cohort", "filter_scoping"],
        ...
        # anti_cohort variant: "no/without B" must generate WHERE db.user_id IS NULL
        EvalCase(
            question="How many users did onboarding and no UPI transactions in Feb",
            expected_analysis_type="behavioral_cohort",
            tags=["sql_structure", "behavioral_cohort", "anti_cohort"],
            gold_qo={
        ...
        # List-valued filter must render as IN (...) not = 'list'
        EvalCase(
            question="show transacting users by platform for iOS and Android last month",
            expected_analysis_type="segment",
            tags=["sql_structure", "filter_rendering", "list_filter"],
            gold_sql={
                "forbidden": ["= '['", "= \"['"],
            },
        ),
        # Bug fix: stickiness must NOT default to app_opened — fixup should hydrate
        # the primary catalog action event (transaction_reconciled for payments category).
        EvalCase(
            question="how sticky is our product",
            expected_analysis_type="stickiness",
            tags=["sql_structure", "stickiness", "generic_event_guard"],
            gold_sql={
                "forbidden": ["event_name = 'app_opened'", "event_name = 'app_open'",
                              "event_name = 'session_start'"],
            },
        ),
        # Bug fix: user_lifecycle must NOT default to app_opened — lifecycle based on
        # app opens is meaningless for a payments product.
        EvalCase(
            question="show me user lifecycle stages",
            expected_analysis_type="user_lifecycle",
            tags=["sql_structure", "user_lifecycle", "generic_event_guard"],
            gold_sql={
                "forbidden": ["event_name = 'app_opened'", "event_name = 'app_open'",
                              "event_name = 'session_start'"],
            },
        ),
        # is_valid() enforcement: journey without an anchor event must clarify, not silently
        # produce a journey anchored on app_opened.
        EvalCase(
            question="show user journey",
            tags=["sql_structure", "journey", "validation_enforcement"],
            # No expected_analysis_type — could clarify or journey. The key assertion is
            # the SQL must not contain app_opened as the anchor.
            gold_sql={
                "forbidden": ["event_name = 'app_opened'"],
            },
        ),
        # activation_rate must use cohort CTE, not scalar ROUND(COUNT/COUNT) hint.
        # The scalar hint ignores qo.filters and can produce rates > 100%.
        EvalCase(
            question="what is activation rate",
            expected_analysis_type="metric",
            tags=["sql_structure", "activation", "pct_users_cte"],
            gold_sql={
                "required": ["WITH denom_cohort", "LEFT JOIN numer_cohort"],
        ...
        EvalCase(
            ...
            expected_analysis_type="metric",
            tags=["sql_structure", "activation", "filter_in_numerator"],
        ...
        # D7 retention: must route correctly and use transaction_reconciled, not app_opened.
        EvalCase(
            question="show D7 retention",
            expected_analysis_type="retention",
            tags=["sql_structure", "retention", "event_hydration"],
            gold_qo={"metric_id": "d7_retention"},
        ),
        # Monthly activation trend: must use cohort bucketing, rates must all be 0-100%.
        EvalCase(
            question="show activation rate MOM for last 6 months",
            expected_analysis_type="metric",
            tags=["sql_structure", "activation", "monthly_trend", "pct_users_cte"],
            gold_sql={
                "required": ["WITH denom_cohort", "cohort_month", "GROUP BY 1"],
                "forbidden": ["COUNT(DISTINCT CASE WHEN"],
            },
            gold_result={
                "col_bounds": {"activation_rate_30d": [0, 100]},
    ...
    # 5) Adversarial / robustness cases (fill remaining slots up to 100)
    # These test graceful degradation: typos, ambiguous intent, bad date windows,
    # out-of-catalog events, multi-intent, and underspecified queries.
    adversarial: list[EvalCase] = [
        ...
        EvalCase(
            "whats my activation rt for jan",
            expected_analysis_type="metric",
            tags=["adversarial", "typo"],
        ),
        EvalCase(
            "transacting usrs by platfrm",
            expected_analysis_type="segment",
            tags=["adversarial", "typo"],
        ),
        # Conflicting / impossible window for metric type
        EvalCase(
            "show MAU for last 3 days",
            tags=["adversarial", "bad_window"],
        ),
        EvalCase(
            "show monthly retention for today",
            tags=["adversarial", "bad_window"],
        ),
        # Ambiguous entity — agent should clarify or pick best guess
        EvalCase(
            "show me users",
        ...
        EvalCase(
            "is our retention good",
            tags=["adversarial", "ambiguous"],
        ),
        # Out-of-catalog events — should trigger clarify or best-effort
        EvalCase(
            "how many users opened the settings page last month",
            tags=["adversarial", "out_of_catalog"],
        ),
        EvalCase(
            "show search bar clicks by platform",
            tags=["adversarial", "out_of_catalog"],
        ),
        # Multi-intent — two metrics in one query
        EvalCase(
            "compare activation rate and churn rate by platform for Q1",
            tags=["adversarial", "multi_intent"],
        ),
        EvalCase(
            "show DAU and MAU together for last 3 months",
            tags=["adversarial", "multi_intent"],
        ),
        # Relative / informal time references
        EvalCase(
        ...
        EvalCase(
            "YTD new user signups",
            expected_analysis_type="metric",
            tags=["adversarial", "relative_time"],
        ),
        EvalCase(
            "show activation rate week over week",
            expected_analysis_type="metric",
            tags=["adversarial", "relative_time"],
        ),
        # Underspecified / garbage
        EvalCase(
            "show metrics",
        ...
        # Negation / exclusion filter
        EvalCase(
            "show DAU excluding android users for Feb",
            expected_analysis_type="metric",
        ...
        EvalCase(
            ...
            expected_analysis_type="metric",
            tags=["adversarial", "filter"],
        ),
        # Implicit comparison (no vs/compare keyword)
        EvalCase(
            "jupiter active users transacting users Feb",
            expected_route="custom_split",
            tags=["adversarial", "implicit_comparison"],
        ),
    ]

    # Fill remaining slots with adversarial cases, then minimal fallback
    remaining = 100 - len(cases)
    for j in range(remaining):
        cases.append(adversarial[j % len(adversarial)])
    return cases[:100]
...
def run_benchmark(
    ...
    *,
    ...
    questions = build_100_question_suite(catalog)
